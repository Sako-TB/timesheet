...
4. Click **Add day** for each day worked. Type the morning and afternoon start/end times in 24-hour format (e.g. `08:30`, `17:00`). Typing `830` also works — it becomes `08:30` when you click away. Lunch is automatically excluded, since it's the gap between the two sessions.
5. Click **Calculate hours** — the button turns green when done.
6. Click **Print / Save PDF**.

One A4 page is printed per employee, each pre-filled with the same dates and times. Signatures are done by hand on the printed pages.

Total hours are shown in `h:mm` format (e.g. `15:00` = 15 hours).
...
- `index.html` — the entire app. JSX is precompiled and version-pinned, so no build step and no runtime compiler.
- `README.md` — this file.
